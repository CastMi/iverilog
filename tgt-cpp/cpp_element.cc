...
vector<cpp_element*> cpp_element::allocated_;

// Just a counter of total bytes allocated for statistics
size_t cpp_element::total_alloc_(0);

void cpp_element::set_comment(std::string comment)
{
   comment_ = comment;
}
 ...
 */
void cpp_element::emit_comment(std::ostream &of, int level,
...
void* cpp_element::operator new(size_t size) throw (bad_alloc)
{
   // Let the default new handle the allocation
   void* ptr = ::operator new(size);

   // Remember this element so we can delete it later
   cpp_element* elem = static_cast<cpp_element*>(ptr);
...
void cpp_element::operator delete(void* ptr)
{
   ...
   vector<cpp_element*>::iterator it =
      find(allocated_.begin(), allocated_.end(), static_cast<cpp_element*>(ptr));
   ...
   if (it != allocated_.end()) {
      *it = NULL;   // It's safe to delete a NULL pointer and much cheaper
      ...
      cerr << "??? cpp_element::operator delete called on an object not "
           << "allocated by cpp_element::operator new" << endl;
   }
}

// Return the total number of bytes our custom operator new has seen.
size_t cpp_element::total_allocated()
{
   return total_alloc_;
}
...
int cpp_element::free_all_objects()
{
   for (vector<cpp_element*>::iterator it = allocated_.begin();
